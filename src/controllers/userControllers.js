...
export const postJoin = async (req, res) => {
    ...
    try{
        ...
        return res.status(400).render("join", { pageTile: "Join", errorMessage: error._message})
    }
...
export const postEdit = async (req, res) => {
    const { 
        session: { user : {_id}, avatarUrl}, body: { name, email, username, location }, file, } = req;
        console.log(file);
...
export const postChangePassword = async (req, res) => {
    const { 
        session: { user : {_id, password}, }, body: { oldPassword, newPassword, newPasswordConfirm } } = req;
    ...
    console.log("nuhash", user.password);
    await user.save();
